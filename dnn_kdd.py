...
data = pd.read_csv('kddcupp99.csv')
...
print(data["lnum_compromised"].value_counts())

print(data["lroot_shell"].value_counts())
print(data["lsu_attempted"].value_counts())
print(data["lnum_root"].value_counts())
print(data["lnum_file_creations"].value_counts())
print(data["lnum_shells"].value_counts())
print(data["lnum_access_files"].value_counts())
...
correlations = correlations.round(2)
correlations.to_csv('./MV_DV_corr_Mat1.csv',index=False)
fig = plt.figure()
...
plt.savefig('./MV_DV_corr_Mat1.jpeg')
...
train_df = train_df.drop(columns=['land','wrong_fragment','urgent','hot','num_failed_logins','lnum_compromised','lroot_shell','lsu_attempted','lnum_file_creations','lnum_shells','lnum_access_files','lnum_outbound_cmds','is_host_login','is_guest_login','serror_rate','rerror_rate'])

train_df.to_csv('check-kdd.csv' , index=False)
...
rf = RandomForestClassifier(n_estimators = 20000,max_depth=5,
